...
nopts = list(
    ...
    breadth = 0.1,
    scale = list(ext = 1.25e-7, col = 6e-6),
    r_use = 5e-4)
...
p1 = ggplot(pldat) +
    geom_line(aes(x = r, y = value, group = species), colour = "#8b72c9", linewidth = lwd) +
    ...
    ggplot2::geom_hline(ggplot2::aes(yintercept = 0), linewidth = 1.2) + ggtitle("A")
# p1
...
pdf(file = "figures/fig_metacom.pdf", width = 7.5, height = 4)
